...
class Landmark:
    def __init__(self):
        ...
        with open('./avatars/ultralight/face_detect_utils/mean_face.txt', 'r') as f_mean_face:
            mean_face = f_mean_face.read()
        self.mean_face = np.asarray(mean_face.split(' '), dtype=np.float32)
        self.det_net = SCRFD('./models/scrfd_2.5g_kps.onnx', confThreshold=0.1, nmsThreshold=0.5)

        checkpoint = torch.load('./models/checkpoint_epoch_335.pth.tar')
