...
#
#  规则:
#    - put_high(item): 插到高优队列**最前端**（紧跟当前正在播的那句之后）
#    - put_low(item):  追加到低优队列右端（话术补位 → 同优先级 FIFO）
#    - get_next():     优先从高优取，高优空则取低优
#    - auto_fill:      低优队列 < min_size 时自动从话术库补位
#
#  注意: 高优队列目前**没有生产者**。弹幕/礼物/关注已改走「直插」通道
#        （api.py 的 push_priority → LiveTalking priority + utt 撤回），不经过队列；
#        低优队列由 auto_fill 持续补话术。保留 put_high 是为将来留口子。
###############################################################################
